...
def test_predict_hybrid_ci_columns_and_interval():
    ...
    def resid_ci_fn(D, cfg, tr, va, te, target, save_path=None):
        # 잔차타깃 = FAIR − MC_hat − recent_margin 이 train/val 에 올바로 전달됐는지 확인
        exp_tr = (D.FAIR[tr] - mc_hat[tr] - D.rm[tr]).astype("float32")
    ...
    np.testing.assert_allclose(df["y_pred"].values,
                               mc_hat[te] + D.rm[te] + np.array([0.01, 0.02]), atol=1e-6)
...
def _fake_full_D(n=24):
    ...
    return SimpleNamespace(
        ...
        FAIR=(0.85 + 0.1 * rng.rand(n)).astype("float32"),
